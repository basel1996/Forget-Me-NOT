...
import { LogIn, User as UserIcon, Plus, Check, X, Sparkles, LogOut, Settings as SettingsIcon, ArrowLeft, Home, User as UserTab, ArrowDown, ArrowUp, Repeat, RotateCcw, Archive, Trash2, Inbox, Play, Trophy, Activity, AlertTriangle, CornerUpLeft, Coffee, Wind, Leaf, Moon, Cloud, Feather, Sun, Mountain, Compass, Waves, Download, UploadCloud, Save } from "lucide-react";
...
export default function App() {
  ...
  const handleManualSave = async () => {
    ...
    try {
      await storageService.saveData({ tasks, routines: [] });
      setToastError("Saved successfully!");
      setTimeout(() => {
        setIsManualSaving(false);
      }, 500);
    } catch (e) {
      console.error(e);
      setToastError("Failed to save data.");
      setIsManualSaving(false);
    }
  };
  ...
  const fetchTasks = async (status: 'active' | 'completed' = 'active') => {
    // Legacy fetch logic replaced by local boot seq
  };

  const fetchCompletedTasksCount = async () => {
    // counts handled implicitly now
  };
  ...
  // 1. BOOT SEQUENCE: Load data from native storage
  useEffect(() => {
  ...
  // 2. THE OPTIMISTIC SAVE TRIGGER: Watch for ANY changes to tasks and silently save them.
  useEffect(() => {
    if (!isLoading) {
      storageService.saveData({
        tasks: tasks,
        routines: [], // If we had separate routine state, we'd save it here
      });
    }
  }, [tasks, isLoading]);
...
const handlePullSubmit = async (e: React.KeyboardEvent<HTMLInputElement>) => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-base text-muted text-xl font-light tracking-wide duration-500">
        Loading your mind...
      </div>
    );
  }
    ...
    <div className={`distraction-free relative min-h-screen transition-colors duration-1000 ${phase === 'night' ? 'night-theme-override' : ''}`}>
      <header className="flex justify-between items-center mb-6 pt-4">
        ...
        <div className="flex gap-2 items-center">
          <button onClick={handleManualSave} disabled={isManualSaving} className="p-2 text-muted hover:text-content transition-colors rounded-full hover:bg-outline/30 flex items-center gap-2" title="Save locally">
            {isManualSaving ? <Repeat size={20} className="animate-spin text-primary" /> : <Save size={20} />}
          </button>
